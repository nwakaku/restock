export const suggestedItems = [
  {
    id: "Olive Oil",
    name: "Olive Oil",
    image:
      "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_691b69ef-443b-4877-a6a2-dcac47144457.jpeg",
    price: 12.99,
  },
  {
    id: "Garlic",
    name: "Garlic",
    image:
      "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_ee41b6c1-96dc-4250-a974-9df18382041f.jpeg",
    price: 1.99,
  },
  {
    id: "Parmesan",
    name: "Parmesan",
    image:
      "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_dfe7b80a-72a1-4115-9189-6213f9fda985.jpeg",
    price: 8.99,
  },
  {
    id: "Basil",
    name: "Basil",
    image:
      "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_b8073b49-7ed5-4979-b3d0-4eefdb10d95f.jpeg",
    price: 2.99,
  },
  {
    id: "Carrot",
    name: "Carrot",
    image:
      "https://www.instacart.com/image-server/394x394/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_369bec92-4389-476b-8e1f-2b0f15698af2.jpeg",
    price: 5.99,
  },
  {
    id: "Oroweat Whole Wheat",
    name: "Oroweat Whole Wheat",
    image:
      "https://www.instacart.com/image-server/394x394/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_f29b39c3-0903-4288-8bd3-f26da58f3242.png",
    price: 4.69,
  },
  {
    id: "Basmatic",
    name: "Basmatic",
    image:
      "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_7a1d3c4d-6771-422f-b18f-de2208a70fed.jpeg",
    price: 2.99,
    unit: 12,
  },
  {
    id: "Lucerne Milk, Whole",
    name: "Lucerne Milk, Whole",
    image:
      "https://www.instacart.com/image-server/394x394/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_61b1f95c-0904-425a-b707-4c3b278be6c6.jpeg",
    price: 6.99,
  },
  {
    id: "Chilled Chicken Tenders",
    name: "Chilled Chicken Tenders",
    image:
      "https://www.instacart.com/image-server/394x394/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_1950f237-7672-45f6-8d25-ac0dd328988a.jpeg",
    price: 14.09,
  },
  {
    id: "Real Mayonnaise",
    name: "Real Mayonnaise",
    image:
      "https://www.instacart.com/image-server/394x394/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_da895e7c-db27-4a47-aeec-7c3c5acf1eda.png",
    price: 2.99,
  },
  {
    id: "Basil",
    name: "Basil",
    image:
      "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_b8073b49-7ed5-4979-b3d0-4eefdb10d95f.jpeg",
    price: 2.99,
  },
];


// Sample generated lists for different prompts
export const sampleLists = {
  bachelor: {
    prompt:
      "Monthly grocery list for a bachelor in Nigeria with ₦100,000 budget",
    budget: 100000,
    items: [
      {
        id: "Rice (Golden Penny)",
        name: "Rice (Golden Penny)",
        quantity: 2,
        unit: "10kg bags",
        price: 15,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_a173753a-d6fd-420e-8915-95db557e33ae.jpeg",
      },
      {
        id: "Spaghetti (Dangote)",
        name: "Spaghetti (Dangote)",
        quantity: 5,
        unit: "500g packs",
        price: 0.5,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_7aaf24e6-dcdd-4eac-b7f3-248efde80c8f.jpeg",
      },
      {
        id: "Eggs",
        name: "Eggs",
        quantity: 2,
        unit: "crates",
        price: 2.5,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_f2a0a010-0b6e-4d94-bd34-c39fa04626b2.jpeg",
      },
      {
        id: "Frozen Chicken",
        name: "Frozen Chicken",
        quantity: 1,
        unit: "kg",
        price: 3,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_a6272567-668e-47d0-a3b9-72a68cf7ea54.jpeg",
      },
    ],
    totalCost: 0,
  },
  family: {
    prompt:
      "Weekly grocery for a family of 4 in Lagos, healthy options, ₦50,000 budget",
    budget: 50000,
    items: [
      {
        id: "Fresh Tomatoes",
        name: "Fresh Tomatoes",
        quantity: 2,
        unit: "kg",
        price: 1.5,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_97975f2d-daaa-476f-a225-5763b8bf71b8.jpeg",
      },
      {
        id: "Green Leafy Vegetables",
        name: "Green Leafy Vegetables",
        quantity: 3,
        unit: "bunches",
        price: 0.5,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_99263696-421c-4c55-aa15-493b3a94fde6.jpeg",
      },
      {
        id: "Rice (Golden Penny)",
        name: "Rice (Golden Penny)",
        quantity: 2,
        unit: "10kg bags",
        price: 15,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_a173753a-d6fd-420e-8915-95db557e33ae.jpeg",
      },
      {
        id: "Spaghetti (Dangote)",
        name: "Spaghetti (Dangote)",
        quantity: 5,
        unit: "500g packs",
        price: 0.5,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_7aaf24e6-dcdd-4eac-b7f3-248efde80c8f.jpeg",
      },
    ],
    totalCost: 0,
  },
  party: {
    prompt: "Weekend party supplies for 20 people, traditional Nigerian dishes",
    budget: 150000,
    items: [
      {
        id: "Goat Meat",
        name: "Goat Meat",
        quantity: 5,
        unit: "kg",
        price: 5,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_8449b9e4-1ca3-4e0e-9a9a-951b39b94592.jpeg",
      },
      {
        id: "Rice (Golden Penny)",
        name: "Rice (Golden Penny)",
        quantity: 2,
        unit: "10kg bags",
        price: 15,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_a173753a-d6fd-420e-8915-95db557e33ae.jpeg",
      },
      {
        id: "Spaghetti (Dangote)",
        name: "Spaghetti (Dangote)",
        quantity: 5,
        unit: "500g packs",
        price: 0.5,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_7aaf24e6-dcdd-4eac-b7f3-248efde80c8f.jpeg",
      },
      {
        id: "Catfish",
        name: "Catfish",
        quantity: 3,
        unit: "kg",
        price: 4,
        note: "",
        image:
          "https://www.instacart.com/image-server/514x514/filters:fill(FFF,true):format(jpg)/d2lnr5mha7bycj.cloudfront.net/product-image/file/large_fea1c4e1-ea01-4827-8635-940632bd2016.jpeg",
      },
    ],
    totalCost: 0,
  },
};